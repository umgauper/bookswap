...
//User.get() and Auth.getCurrentUser()._id were returning undefined, so I used a direct get request to user api instead
      ...
      $scope.submitTrade = function (bookID, bookName) {
        $http.put('api/books/' + bookID, {user: $scope._id, book: bookName}).success(function () {
          $window.location.reload();
        });
      };

      $scope.acceptTrade = function (bookID) {
        $http.put('api/books/acceptTrade/' + bookID).success(function () {
          $window.location.reload();
        })
      };
    });
